// Common bootstrap for all hwp-skill scripts (core/ and enhanced/).
//
// Responsibilities:
...
// shell out to the rhwp CLI binary (`enhanced/render.mjs`), which uses
...
export async function loadDocumentFromBytes(bytes) {
  await ensureInit();
  return new HwpDocument(new Uint8Array(bytes));
}
...
// agent should be steered to extract_tables.mjs.
//
// The implementation moved to lib/doc_walk.mjs, which owns paragraph walking
// and control classification for the whole skill; it is re-exported here so
// existing importers keep working.
export { documentHasTable } from "./doc_walk.mjs";
