...
## 11. rhwp의 스타일(스타일) 지원 범위 (2026-08-27)

한 줄 요약: **rhwp는 스타일 테이블을 읽기 전용으로 다룬다.**
문서에 이미 있는 스타일을 **읽고 적용**할 수 있고, **정의(생성·수정·삭제)는 전부 안 된다.**

### 11-1. 읽기 — 완전하다

| API | 반환 |
|---|---|
| `getStyleList()` | `[{id, name, englishName, type, nextStyleId, paraShapeId, charShapeId}]` |
| `getStyleDetail(id)` | `{charProps, paraProps}` — 해석된 전체 속성 |
| `getStyleAt(s,p)` | `{id, name}` |
| `getCellStyleAt(s,p,c,cell,cellPara)` | 〃 (셀 안) |

`type` 값: **0 = 문단 스타일, 1 = 글자 스타일**. 기본 문서엔 `쪽 번호`(id 12)만 type 1이다.
실문서에서는 `0`(2174) `1`(49) 외에 **`4`(11) `108`(25)** 도 관측됐다 — 미해석 값이 있다.

스타일 개수는 문서마다 다르다: `fixture-table.hwp` **14개**, `fixture-headings.hwp` 22개,
실문서 **median 31개 / max 119개**. **"기본 22개"를 가정하면 안 된다** —
`applyStyle`에 범위 밖 id를 주면 throw한다(조용하지 않음, 좋다).

### 11-2. 적용 — 동작한다. 이전 판정을 정정한다

**정정:** §5에서 "`applyStyle`은 문자 서식을 안 옮긴다"고 적었으나 **틀렸다.**
`개요 1`(charShapeId=0)로 시험해서 바꿀 것이 없었을 뿐이다. charShape이 실제로 다른
스타일로 재시험한 결과:

| 적용한 스타일 | 문자 적용 | 문단 적용 | 실측 |
|---|---|---|---|
| 2 `개요 1` (charSh 0) | – | ✅ | 바꿀 것 없음 |
| **12 `쪽 번호` (type 1)** | ✅ | ❌ | **글자 스타일이라 문단은 안 건드림 — 올바른 의미론** |
| 14 `각주` (charSh 3) | ✅ | ✅ | fontSize 1000 → **900** |
| **17 `차례 제목` (charSh 5)** | ✅ | ✅ | **글꼴 함초롬바탕 → 함초롬돋움**, 1000 → **1600** |

**→ 글꼴은 `applyStyle`로 바뀐다.** §6의 "어떤 경로로도 안 된다"는 **정정되어야 한다.**
정확한 진술은: *임의의* 글꼴로는 못 바꾸고, **문서에 이미 있는 스타일이 가진 글꼴로는 바뀐다.**
...
| 스타일로 도달 가능한 글꼴 수 | 7 |
| 문서가 실제 쓰는 글꼴 수 | 10 |
| **문서가 쓰는 글꼴 중 스타일로 도달 가능한 비율** | **64%** |
| 100% 도달 가능한 문서 | **1/58 (2%)** |

기타 적용 API:

| API | 결과 |
|---|---|
| `applyCellStyle(s,p,c,cell,cellPara,styleId)` | ✅ 시그니처 확정, 디스크 확인 |
| `setParaShapeId(s,p,id)` | ✅ |
| `setCellParaShapeId(s,p,c,cell,cellPara,id)` | ✅ |
| `setCharShapeId(...)` | ❌ 전 id 무동작 (§6) |
| HWPX 입력 | ✅ `getStyleList` 24개, `applyStyle` ok |

### 11-3. 정의 — 전부 안 된다

| API | 반환 | 실제 |
|---|---|---|
| `createStyle(json)` | 새 id (숫자) | **이름만 생긴다.** `charProps`·`paraProps`·`charShapeId`·`paraShapeId` 전부 무시 → 빈 껍데기 |
| `updateStyle(id, json)` | `true` | **메모리에서조차 무변화.** `getStyleDetail`이 그대로다 |
| `updateStyleShapes(id,p,c)` | — | **WASM `memory access out of bounds`** |
| `deleteStyle(id)` | `true` | **목록이 안 줄어든다.** 그런데 그 스타일을 쓰던 문단은 `바탕글`로 되돌아간다 → **부분 적용. 위험하므로 쓰지 말 것** |

`createStyle({})` 는 빈 객체로도 성공해 이름 없는 스타일을 만든다.

### 11-4. 결론

스킬이 노출할 수 있는 것은 **스타일 목록 읽기 + 적용**뿐이다.
"이 문서의 제목 스타일을 다른 문단에도 입혀 줘"는 된다.
"제목 스타일을 만들어 줘 / 글꼴을 바꿔 줘"는 **안 된다** — 명시적으로 거절해야 한다.

이건 §10-3의 설계 원칙과 잘 맞는다: 스타일 이름은 **문서 자신의 어휘**이므로
`applyStyle`의 주소로 그대로 쓸 수 있고, 우리가 발명할 것이 없다.
다만 §10-5대로 커버리지가 부족하므로(85% 문서에서 shape > style) 스타일은
**보조 축**이지 표현의 기반이 아니다.

---

## 12. 보고된 known failure 3건 조사 (2026-08-27)

### 12-1. 삽입 시 앵커 서식 상속 — 확인, 그리고 고칠 수 있다

`insertText(sec, para, offset, text)` 에는 **서식 인자가 없다.** 상속은 구조적이다.

**정확한 규칙 (실측):** 삽입된 문자는 **바로 앞 글자(offset-1)의 서식**을 상속한다.
offset 0에서만 뒤쪽 글자를 따른다. 한 문단을 `[0,8)=bold 18pt` / `[8,16)=italic 9pt`로
갈라 놓고 offset 0·4·8·12·16에 삽입해 확인:

| 삽입 offset | 상속한 서식 |
|---|---|
| 0, 4, 8 | 앞 run (bold 1800) |
| 12, 16 | 뒤 run (italic 900) |

offset 8은 경계인데 **앞쪽**을 따랐다 — "앞 글자" 규칙과 일치한다.

**해법: 삽입 후 삽입 범위에만 `applyCharFormat`.** 검증됨. 단 범위 계산에 함정이 있다.

| 삽입 텍스트 | `js.length` | codepoints | **엔진 길이 변화** | |
|---|---|---|---|---|
| `hello` | 5 | 5 | 5 | ok |
| `한글삽입` | 4 | 4 | 4 | ok |
| **`테스트🙂끝`** | **6** | 5 | **5** | **`js.length`는 틀린다** |
| `a\tb` | 3 | 3 | 3 | ok (단 탭 자체는 서식이 안 먹음: 3자 중 2자만) |

→ **범위를 `off + text.length`로 계산하면 안 된다.** 서로게이트 페어에서 1자씩 넘친다.
**`getParagraphLength`를 삽입 전후로 재서 차이를 쓰는 것**이 유일하게 안전하다
(엔진에게 묻는 것이므로 가정이 없다).

bold·italic·underline·strikethrough는 §3의 33키 스윕에서 전부 동작 확인됨.

### 12-2. 불렛 — rhwp 한계가 아니라 **호출 규약 문제였다**

`ensureDefaultBullet`은 **JSON이 아니라 생짜 문자열을 받는다.**

| 인자 | 결과 |
|---|---|
| `ensureDefaultBullet("●")` | ✅ `{id:1, char:"●", rawCode:9679}` |
| `ensureDefaultBullet('{"char":"●"}')` | ❌ `{id:1, char:"{", rawCode:123}` — **첫 글자를 불렛으로 삼는다** |
| `ensureDefaultBullet(9679)` | 💥 **WASM `memory access out of bounds`** |
| `ensureDefaultBullet({})` | 💥 동일 |
| `ensureDefaultBullet("●●")` | 첫 글자만 사용 |
| `ensureDefaultBullet("")` | 미정의 동작 — 거부해야 함 |

앞선 §5에서 "`ensureDefaultBullet`은 저장 안 됨"이라 적은 것은 **두 겹의 오류**였다:
JSON을 넘겨 `{`가 만들어졌고, 게다가 참조되지 않아 pruning됐다.

**동작 확인:** 한 문서에 `□ ○ - ●` 4종을 만들고 문단 4개에 `paraLevel` 0~3으로 적용 →
전부 디스크에 남았다.

```
ensureDefaultBullet("□") -> id
applyParaFormat(s, p, {headType:"Bullet", numberingId:id, paraLevel:n})
```

`createNumbering`과 동일하게 **문단이 참조해야 저장된다.**
기존 문서의 불렛 id를 그대로 재사용하는 것도 확인됨.

**단, 실문서에서 HWP 불렛 기능은 드물다:**

| | |
|---|---|
| `headType != None` | 147/3503 (4.2%) — 이 중 Bullet 146 |
| 진짜 Bullet을 가진 문서 | **4/70** |
| **글리프를 그냥 본문 텍스트로 쓴 문단** | **274 (7.8%)** — 약 2배 |

→ 한국 문서의 "불렛"은 대개 **글자 `□`/`○`/`-` + 들여쓰기**지 HWP 불렛 기능이 아니다.
두 경로를 **모두** 다룰 수 있어야 한다.

### 12-3. 들여쓰기 레벨 — 가정을 수정해야 한다

**`marginLeft`은 실문서에서 레벨을 나누는 데 쓰이지 않는다.**

| | |
|---|---|
| `marginLeft == 0` 인 문단 | **1702/1994 (85%)** |
| 마커 글리프별 marginLeft median | **전부 0** (□ ○ - ※ ◦ * ㅇ `N.`) |
| **문서 안에서 marginLeft이 마커들을 구분한 비율** | **0/3 (0%)** |

마커 문단 376개의 깊이 신호 출처:

| 출처 | |
|---|---|
| 문단 지오메트리 (`marginLeft>0` 또는 `indent≠0`) | 260 (69%) |
| 본문 앞 공백만 | 47 (13%) |
| 아무 신호 없음 | 69 (18%) |

지오메트리 쪽의 지배적 수단은 `marginLeft`이 아니라 **음수 `indent`(내어쓰기)** 다:
◦ 94% / ㅇ 94% / * 69% / □ 59%가 hanging indent를 쓴다.

**두 가지가 서로 다른 일을 한다:**
- **본문 앞 공백** = 레벨(깊이). 마커별로 일관된다 (□=1, ◦=2, ○=3 …)
- **음수 `indent`** = 줄바꿈된 뒷줄을 마커 뒤 텍스트에 맞추는 **정렬**. 깊이가 깊어지면
  같이 깊어진다(둘이 결합돼 있다)

관측 예 (문서 내부):
```
□ ◦ * -  ->  ws [1,2,4,4]   ml [0,0,0,0]     ind [0,-48.8,-49.9,-52.8]
□ ㅇ -    ->  ws [0,1,5]     ml [16,0,0]      ind [-29.3,-39,-65.7]
```

**측정의 한계를 밝힌다:** 마커 2종 이상을 각 3회 이상 쓴 문서가 **3건뿐**이었다.
"marginLeft이 레벨을 안 나눈다"는 결론은 전체 집계(85% 0, 마커별 median 0)로
뒷받침되지만, "그럼 무엇이 나누는가"의 답은 **표본 3건에 기대고 있다.**
설계를 확정하기 전에 더 큰 표본이 필요하다.

`marginLeft` `indent` 둘 다 편집은 정상 동작한다(§4). 배치 적용을 막을 엔진 제약은 없다.

### 12-3b. 들여쓰기 — 표본을 키워 재측정 (§12-3의 약한 주장을 대체한다)

§12-3의 "무엇이 깊이를 나누는가"는 문서 3건에 기댄 주장이었다. 마커 문단
**1,328개 / 52개 문서**로 다시 쟀다. **결론은 유지되고, 근거가 바뀐다.**

| 마커 문단에 존재하는 신호 | |
|---|---|
| **본문 앞 띄어쓰기** | **506 (38%)** |
| `marginLeft > 0` | 182 (14%) |
| 음수 `indent` (내어쓰기) | 528 (40%) ← 깊이가 아니라 **정렬** |
...
문서 단위(마커 문단 4개 이상인 52개 문서):

| | |
|---|---|
| 띄어쓰기만 쓰는 문서 | 17 (33%) |
| `marginLeft`만 쓰는 문서 | 8 (15%) |
| 둘 다 나타나는 문서 | 8 (15%) |
| 둘 다 없는 문서 | 19 (37%) |

**사용된 공백 문자:** `SPACE` **1,137** / `TAB` **3**.
전각 공백(U+3000)도 NBSP도 사실상 안 쓰인다 — **그냥 스페이스바다.**

**요약:** 띄어쓰기가 `marginLeft`보다 **3배** 흔하다. 그러나 절반(50%)은
**어떤 들여쓰기 신호도 없다** — 깊이를 마커 글리프 자체(`□` vs `○`)가 지고 있고
들여쓰기는 선택적 보강이다. 이는 `headings.mjs`가 이미 마커 글리프로 깊이를
학습하는 설계와 일치한다.

실제 예 (공백을 `·`로 표시):
```
   1.·사업근거                          ml=0     ind=0
  ·□·「과학기술기본법」·제23조…          ml=0     ind=-37.7
  ···○·대한민국·국적·소지자로,…          ml=0     ind=-59.1
```
```
   1.·연구과제의·개요                    ml=0     ind=0
  ··1)·주요·연구·변경사항                ml=0     ind=0      <- 지오메트리 전혀 없음
```

**설계 함의:** "들여쓰기 레벨 조정"은 단일 연산이 아니다. 문서가 쓰는 방식을 먼저
판별하고 **그 문서의 방식대로** 적용해야 한다. `marginLeft`만 건드리는 API는
실문서의 37%(띄어쓰기만 쓰는 문서 33%)에서 눈에 보이는 효과가 없거나 방식을 섞어
문서를 일관성 없게 만든다.
