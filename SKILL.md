---
name: hwp-for-dobby
description: "Use this skill whenever the user wants to view, read, edit, fill, create, OUTLINE, or SECTION-EXTRACT Korean HWP/HWPX documents (.hwp, .hwpx — the native format of Hancom Office and the de facto standard for Korean public-sector forms). Triggers include: opening or summarizing an .hwp; finding/replacing text; filling a Korean government form (신청서, 보고서, 양식); building a new .hwp from scratch; extracting text/tables/images; converting to text/markdown/PDF/PNG; inspecting structure. ALSO use for STRUCTURE work on an .hwp: 'extract section 4.3', 'give me §3.2 / 제12조', '4.3절만 뽑아줘', outline/map/목차 this document, split a document by section or heading for per-section review or translation, feed sections to subagents one at a time, and 'what changed since last week' / '지난주 대비 뭐가 바뀌었어' (section-level snapshot + diff). ALSO use to READ an .hwp with TRACKED CHANGES (변경 내용 추적) — a plain reader silently mixes deleted text into the body — and to read memos/주석, footnotes and equations. Triggers on '한글 문서', '한컴 한글', 'HWP', 'HWPX', or casual mentions like 'the hwp in my downloads' / 'this 보고서 양식 they sent me' / '이거 한글 파일'. Use this skill even when the user does not explicitly say HWP, as long as the file extension is .hwp/.hwpx or the document is described as a Hangul/Hancom file. Do NOT use for .docx (use the docx skill or docx-section-extractor), .xlsx (use xlsx), .pptx (use pptx), .pdf (use pdf), or .odt — only HWP and HWPX."
