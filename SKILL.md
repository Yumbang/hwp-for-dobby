...
- **Find/replace / structural edits**: save as `.hwp`. One residual engine bug remains, now precisely localized (verified empirically 2026-06): **edits to plain body paragraphs roundtrip fine; edits to text inside tables or textboxes of an `.hwp`-sourced document are silently dropped on save.** `.hwpx`-sourced documents do NOT have this bug — their `.hwp` export materializes fully from the edited IR, in-table edits included. So: body edit on `.hwp` → expect success; in-table edit on `.hwp` → expect `verified: false` and tell the user the engine can't do it yet; any edit on `.hwpx` input → expect success. `replace.mjs` auto-detects every failure on reload (`verified: false` = failed task). There is no `.hwpx`-output fallback.
- **Form filling**: save as `.hwp`. **Empty fields fill reliably** (verified: values roundtrip in both the field API and rendered text). **Pre-populated fields are Hancom-suspect**: the current engine build replaces field text without shifting char-shape/line-seg metadata (upstream #838, fixed on rhwp devel, not in our build) — rhwp reads the result back fine, but Hancom Office may reject it as corrupted. Warn the user when filling a field that already has text. Filled values may also inherit placeholder styling (red/bold/italic) — visually verify the output.
